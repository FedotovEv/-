...
public:
    ...
    virtual double Evaluate(GetCellPtr get_cell) const = 0;
    ...
    static constexpr double ZERO_TOLERANCE = 0.00000001;
};
...
public:
    ...
    double Evaluate(GetCellPtr get_cell) const override
    {
        double result;
        double lhs_arg = lhs_->Evaluate(get_cell);
        double rhs_arg = rhs_->Evaluate(get_cell);
...
public:
    ...
    double Evaluate(GetCellPtr get_cell) const override
    {
        double real_operand = operand_->Evaluate(get_cell);
        switch(type_)
        {
            case UnaryPlus:
                return real_operand;
...
public:
    ...
    double Evaluate(GetCellPtr get_cell) const override
    {
        ...
        CellInterface* cell_ptr = get_cell(*cell_);
        if (!cell_ptr) // Ячейка пуста
            return 0;
        // Ячейка непуста
        CellInterface::Value cell_val = cell_ptr->GetValue();
        if (std::holds_alternative<FormulaError>(cell_val)) // Возвращена ячейка с ошибкой FormulaError
            throw std::get<FormulaError>(cell_val);
        if (std::holds_alternative<std::string>(cell_val))
        { // Строковая ячейка
            std::string cell_string = std::get<std::string>(cell_val);
            std::string::size_type non_space_first = cell_string.find_first_not_of(' ');
            std::string::size_type non_space_last = cell_string.find_last_not_of(' ');
            if (cell_string.size() == 0 || non_space_first == std::string::npos || non_space_last == std::string::npos)
                return 0;
            cell_string = cell_string.substr(non_space_first, non_space_last - non_space_first + 1);
            size_t symb_conv_count;
            double cell_double_val;
            try
            {
                cell_double_val = std::stod(cell_string, &symb_conv_count);
            }
            catch (...)
            {
                throw FormulaError(FormulaError::Category::Value);
            }
            if (symb_conv_count != cell_string.size())
                throw FormulaError(FormulaError::Category::Value);
            else
                return cell_double_val;
        }
        else
        { // Ячейка с числом
...
public:
    ...
    double Evaluate(GetCellPtr) const override
    {
        return value_;
    }
...
double FormulaAST::Execute(GetCellPtr get_cell) const
{
