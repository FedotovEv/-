...
Весь проект разделён на две части - *фронтенд* и *бэкенд*. Обе части обладают существенной независимостью друг
...
1. CMake 3.18.0
2. Исполнительная среда Java(TM) SE Runtime Environment 1.8.0
3. ANTLR 4.7.2
4. Желателен Java Development Kit (JDK) 1.8.0

Для сборки демонстрационной программы:
...
Простейший пример подключения и использования описываемого движка можно найти в файле main.cpp. Там же располагаются
модульные тесты, проверяющие корректность функционирования электронной таблицы. Более развёрнутый и практичный пример
работы с движком находится в приложенной к проекту демонстрационной программе, размещающейся в каталоге SpreadSheetFrontend.
Достаточно подробное описание программного интерфейса библиотеки и способов её применения приведено ниже.
